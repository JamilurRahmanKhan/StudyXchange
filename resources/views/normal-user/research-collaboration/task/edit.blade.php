        ...
        <div class="main-content project">
            <div class="row">
                <div class="col-12">
                    <div class="box">
                        <div class="box-body">
                            <form action="{{route('normal-user.research-task.update',['id'=>$task->id])}}" method="POST" enctype="multipart/form-data">
                                @csrf
                                <div class="row">
                                    <div class="col-md-6 col-sm-12 mb-24">
                                        <div class="form-group">
                                            <label class="form-label">Task Title</label>
                                            <input class="form-control" name="title" value="{{$task->title}}" placeholder="Task Title">
                                            @error('title')
                                            <span class="text-danger" style="font-size: 14px;">{{ $message }}</span>
                                            @enderror
                                        </div>
                                    </div>
                                    <div class="col-md-6 col-sm-12 mb-24 form-group">
                                        <label class="form-label">Deadline:</label>
                                        <div class="input-group">
                                            <div class="input-group-prepend">
                                                <div class="input-group-text"> <i class='bx bx-calendar'></i> </div>
                                            </div>
                                            <input id="datepicker" class="form-control fc-datepicker" value="{{ \Carbon\Carbon::parse($task->due_date)->format('d-m-Y') }}" name="due_date" placeholder="DD-MM-YYYY" type="text">
                                            @error('due_date')
                                                ...
                                                <div class="input-group mb-3">
                                                    <select name="research_team_member_id" class="form-control custom-select select2 select2-hidden-accessible" data-placeholder="Select Client" tabindex="-1" aria-hidden="true" data-select2-id="select2-data-38-9jkg">
                                                        ...
                                                        @foreach($teamMembers as $teamMember)
                                                            <option value="{{ $teamMember->id }}" {{ old('team_member_id', $task->research_team_member_id	) == $teamMember->id ? 'selected' : '' }}>{{ $teamMember->user->name }}</option>
                                                        @endforeach
                                                    </select>
                                ...
                                <div class="row">
                                    <div class="col-md-6 col-sm-12">
                                        ...
                                        <!-- Display Previous Attachment if Exists -->

                                        <div class="mt-3">
                                            <h6 class="text-secondary">Current Attachment:</h6>
                                            <a href="{{ asset($task->attachment) }}" target="_blank" class="btn btn-outline-primary">
                                                <i class="bx bx-link-external me-2"></i> View / Download Attachment
                                            </a>
                                        </div>
                                    ...
                                    <div class="col-md-6 col-sm-12">
                                        <div class="custom-controls-stacked d-lg-flex align-items-center">
                                            ...
                                            <label class="custom-control custom-radio success me-4">
                                                <input type="radio" class="custom-control-input" name="status" value="1" {{ old('status', $task->status) == 1 ? 'checked' : '' }}>
                                                <span class="custom-control-label">Pending</span>
                                            </label>

                                            <!-- On Progress -->
                                            <label class="custom-control custom-radio success me-4">
                                                <input type="radio" class="custom-control-input" name="status" value="2" {{ old('status', $task->status) == 2 ? 'checked' : '' }}>
                                                <span class="custom-control-label">On Progress</span>
                                            </label>

                                            <!-- Completed -->
                                            <label class="custom-control custom-radio success me-4">
                                                <input type="radio" class="custom-control-input" name="status" value="3" {{ old('status', $task->status) == 3 ? 'checked' : '' }}>
